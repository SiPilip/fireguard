...
const MapWithNoSSR = dynamic(() => import('@/components/ReportMap'), {
  ssr: false,
  loading: () => <div className="h-full w-full animate-pulse bg-gray-200">Memuat Peta...</div>,
});
...
export default function NewReportPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!position) {
      setError('Lokasi belum ditentukan. Klik tombol untuk mendapatkan lokasi Anda.');
      return;
    }
    if (!file) {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="container mx-auto p-4">
        <form onSubmit={handleSubmit} className="rounded-lg bg-white p-8 shadow">
          <div className="mb-6">
            <label className="mb-2 block text-lg font-medium text-gray-800">Lokasi Kejadian</label>
          ...
          <div className="mb-6">
            <label htmlFor="media" className="mb-2 block text-lg font-medium text-gray-800">Bukti Foto/Video</label>
            <input
              type="file"
              id="media"
