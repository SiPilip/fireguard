...
export async function POST(request: NextRequest) {
  try {
    // Rate limiting
    const limit = enforceRateLimit(request, "notifications-register", 10, 60_000);
    if (!limit.allowed) {
      return jsonWithCors(
    ...
    await execute(
      ...
      [userId, deviceToken.trim(), platformValue, currentTimestamp, currentTimestamp, currentTimestamp]
    );
